...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros(N, F, int(1+(H+2*pad-HH)/stride), int(1+(W+2*pad-WW)/stride), device=x.device, dtype=x.dtype)
        ...
        for n in range(N): # Loop over batch
            yi = 0
            for hi in range(0, Hpad-HH+1, stride):
                xi = 0
                for wi in range(0, Wpad-WW+1, stride):
                    rec_field = x_pad[n, :, hi:hi+HH, wi:wi+WW]  # (C,HH,WW)
                    for f in range(F): # Loop over filters
                        out[n, f, yi, xi] = torch.sum(rec_field * w[f]) + b[f]
                    xi += 1
                yi += 1
        #####################################################################
        #                          END OF YOUR CODE                         #
    ...
    def backward(dout, cache):
        ...
        for f in range(F):
            for c in range(C):
                for hi in range(HH):
                    for wi in range(WW):
                        dw[f, c, hi, wi] = torch.sum(dout[:,f] * x_pad[:, c, hi:hi+Hout*stride:stride, wi:wi+Wout*stride:stride])
                    ...
                    for wi in range(W):
                        for hout in range(Hout):
                            for wout in range(Wout):
                                hmask = torch.zeros_like(w[f])  # (C,HH,WW)
                                wmask = torch.zeros_like(w[f])  # (C,HH,WW)
                                hindex = hi + pad - hout*stride
                                windex = wi + pad - wout*stride
                                if (0 <= hindex and hindex < HH):
                                    hmask[:, hindex] = 1
                                if (0 <= windex and windex < WW):
                                    wmask[:, :, windex] = 1

                                dx[n, :, hi, wi] += dout[n, f, hout, wout] * torch.sum(w[f]*hmask*wmask, dim=(1,2))
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros(N, C, int(1+(H-pool_h)/stride), int(1+(W-pool_w)/stride), device=x.device, dtype=x.dtype)
        for n in range(N):
            yi = 0
            for hi in range(0, H-pool_h+1, stride):
                xi = 0
                for wi in range(0, W-pool_w+1, stride):
                    rec_field = x[n, :, hi:hi+pool_h, wi:wi+pool_w]  # (C,pool_h,pool_w)
                    for c in range(C): # Loop over channels
                        out[n, c, yi, xi] = torch.max(rec_field[c])
                    xi += 1
                yi += 1
        ####################################################################
        #                         END OF YOUR CODE                         #
        ...
        for n in range(N):
            ...
            for hi in range(0, H-pool_h+1, stride):
                ...
                for wi in range(0, W-pool_w+1, stride):
                    rec_field = x[n, :, hi:hi+pool_h, wi:wi+pool_w]  # (C,pool_h,pool_w)
                    for c in range(C): # Loop over channels
                        dx[n, c, hi:hi+pool_h, wi:wi+pool_w] += dout[n, c, yi, xi] * (rec_field[c] == torch.max(rec_field[c]))
                    xi += 1
                yi += 1
        ####################################################################
        #                          END OF YOUR CODE                        #
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        conv_flattened = int(num_filters * H/2 * W/2)
        self.params['W2'] = weight_scale * torch.randn(conv_flattened, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        dx3, grads['W3'], grads['b3'] = Linear.backward(d_scores, cache3)
        ...
        dx1, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dx2, cache1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        C, H, W = input_dims
        L = len(num_filters) + 1  # num of Conv layers
        kernel = 3
        ...
        if weight_scale == "kaiming":
            for i in range(L-1):
                if i in max_pools:
                    H, W = int(1+(H-2)/2), int(1+(W-2)/2)
                chan = C if (i == 0) else num_filters[i-1]
                self.params['W'+str(i+1)] = kaiming_initializer(chan, num_filters[i], K=kernel, device=device, dtype=dtype)
                self.params['b'+str(i+1)] = torch.zeros(num_filters[i], dtype=dtype, device=device)

            conv_flattened = int(num_filters[-1] * H * W)
            self.params['W'+str(L)] = kaiming_initializer(conv_flattened, num_classes, dtype=dtype, device=device)
            ...
            for i in range(L-1):
                if i in max_pools:
                    H, W = int(1+(H-2)/2), int(1+(W-2)/2)
                chan = C if (i == 0) else num_filters[i-1]
                self.params['W'+str(i+1)] = weight_scale * torch.randn(num_filters[i], chan, kernel, kernel, dtype=dtype, device=device)
                self.params['b'+str(i+1)] = torch.zeros(num_filters[i], dtype=dtype, device=device)

            conv_flattened = int(num_filters[-1] * H * W)
            self.params['W'+str(L)] = weight_scale * torch.randn(conv_flattened, num_classes, dtype=dtype, device=device)
            self.params['b'+str(L)] = torch.zeros(num_classes, dtype=dtype, device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        #########################################################
        layers = []
        if not self.batchnorm:  # No Batchnorm
            for i in range(self.num_layers-1):
                if i in self.max_pools:
                    layers.append(Conv_ReLU_Pool)
                else:
                    layers.append(Conv_ReLU)
            layers.append(Linear)
        else:  # Batchnorm
            for i in range(self.num_layers-1):
                if i in self.max_pools:
                    layers.append(Conv_BatchNorm_ReLU_Pool)
                else:
                    layers.append(Conv_BatchNorm_ReLU)
            layers.append(Linear)
        ...
        for i in range(self.num_layers):
            if (i == self.num_layers-1):
                scores, cache['c'+str(i+1)] = layers[i].forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
            else:
                if not self.batchnorm:
                    if i in self.max_pools:
                        scores, cache['c'+str(i+1)] = layers[i].forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)], conv_param, pool_param)
                    else:
                        scores, cache['c'+str(i+1)] = layers[i].forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)], conv_param)
                else:
                    if i in self.max_pools:
                        scores, cache['c'+str(i+1)] = layers[i].forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)], self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], conv_param, self.bn_params[i], pool_param)
                    else:
                        scores, cache['c'+str(i+1)] = layers[i].forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)], self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], conv_param, self.bn_params[i])
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if (i != self.num_layers and self.batchnorm):
                d_scores, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = layers[i-1].backward(d_scores, cache['c'+str(i)])
            else:
                d_scores, grads['W'+str(i)], grads['b'+str(i)] = layers[i-1].backward(d_scores, cache['c'+str(i)])
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    model = DeepConvNet(input_dims=(data_dict['X_train'].shape[1], data_dict['X_train'].shape[2], data_dict['X_train'].shape[3]),
                        num_classes=10,
                        num_filters=([8] * 2) + ([32] * 1) + ([16] * 2),
                        max_pools=[1, 2, 4],
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # and device.                                                     #
        ###################################################################
        weight = (gain/Din)**0.5 * torch.randn(Din, Dout, dtype=dtype, device=device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        assert K > 0
        weight = (gain/Din/K/K)**0.5 * torch.randn(Dout, Din, K, K, dtype=dtype, device=device)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sample_var = torch.var(x, unbiased=False, dim=0)
            ...
            cache['x'], cache['x_norm'] = x, x_norm
            cache['gamma'], cache['beta'], cache['eps'] = gamma, beta, eps
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        dsample_var = torch.sum(dxhat*(cache['x']-cache['sample_mean'])*(-0.5)*torch.pow(cache['sample_var']+cache['eps'], -1.5), dim=0)
        dsample_mean = -torch.sum(dxhat*torch.pow(cache['sample_var']+cache['eps'], -0.5), dim=0) + dsample_var / N * torch.sum(-2*(cache['x']-cache['sample_mean']), dim=0)
        
        dx = dxhat*torch.pow(cache['sample_var']+cache['eps'], -0.5) + dsample_var*2*(cache['x']-cache['sample_mean'])/N + dsample_mean/N
        dgamma = torch.sum(dout * cache['x_norm'], dim=0)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        reshaped_x = x.permute(0,2,3,1).reshape(N*H*W, C)  # (N,H,W,C) -> (N*H*W,C)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        N, C, H, W = dout.shape
        reshaped_dout = dout.permute(0,2,3,1).reshape(N*H*W, C)  # (N,H,W,C) -> (N*H*W,C)
        dx, dgamma, dbeta = BatchNorm.backward(reshaped_dout, cache)
