...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for ii in range(num_train):
        for jj in range(num_test):
            dists[ii][jj] = torch.sum(torch.square(x_train[ii].flatten()-x_test[jj].flatten()))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for ii in range(num_train):
        dists[ii] = torch.sum(torch.square(x_train[ii]-x_test), dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.flatten(start_dim=1)
    x_test = x_test.flatten(start_dim=1)
    train_squared = torch.sum(x_train**2, dim=1)
    test_squared = torch.sum(x_test**2, dim=1)
    train_times_test = torch.mm(x_train, x_test.T)
    dists = train_squared.unsqueeze(1) - 2*train_times_test + test_squared.unsqueeze(0)
    # dists = torch.sum(torch.square(torch.unsqueeze(x_train, 1)-x_test), dim=2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    nearest_idxs = torch.topk(-dists, k=k, dim=0).indices
    topk_labels = y_train[nearest_idxs]
    y_pred = torch.mode(topk_labels, 0).values
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    # HINT: torch.chunk                                                      #
    ##########################################################################
    x_train_folds = torch.stack(x_train.chunk(num_folds))
    y_train_folds = torch.stack(y_train.chunk(num_folds))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    # HINT: torch.cat                                                        #
    ##########################################################################
    all_accs = [[] for i in range(len(k_choices))]
    for i in range(num_folds):
        classifier = KnnClassifier(torch.cat([x_train_folds[:i], x_train_folds[i+1:]], 0).flatten(start_dim=0, end_dim=1), torch.cat([y_train_folds[:i], y_train_folds[i+1:]], 0).flatten(start_dim=0, end_dim=1))
        for j, k in enumerate(k_choices):
            all_accs[j].append(classifier.check_accuracy(x_train_folds[i], y_train_folds[i], k=k, quiet=True))
    for j, k in enumerate(k_choices):
        k_to_accuracies[k] = all_accs[j]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
