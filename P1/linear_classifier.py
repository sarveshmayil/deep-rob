...
def svm_loss_vectorized(
    ...
    margin = scores - correct_class_scores.expand_as(scores)  # (N,C)
    margin = torch.maximum(torch.zeros_like(margin), margin + torch.ones_like(margin))
    margin[torch.arange(num_train),y] = 0  # Where j == y[i]
    ...
    vals[torch.arange(num_train),y] = -sums.T
...
def sample_batch(
    ...
    #########################################################################
    rand_idxs = torch.randint(low=0, high=num_train-1, size=(batch_size,))
    X_batch = X[rand_idxs]
    y_batch = y[rand_idxs]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def test_one_param_set(
    ...
    train_acc = (data_dict['y_train'] == y_train_pred).double().mean().item()

    y_val_pred = predict_linear_classifier(cls.W, data_dict['X_val'])
    val_acc = (data_dict['y_val'] == y_val_pred).double().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])  # (C,)
        scores -= scores.max()
        softmax_scores = torch.log(torch.exp(scores) / torch.sum(torch.exp(scores)))  # (C,)
        loss -= softmax_scores[y[i]]
        sum_j = 0.0
        for j in range(num_classes):
            sum_j += torch.exp(softmax_scores[j])
        loss += torch.log(sum_j)

        ## dL/dW = dL/df * df/dW
        ## df/dW = X[i]
        for j in range(num_classes):
            dLdf = torch.exp(softmax_scores[j])/sum_j
            if j == y[i]:
                dW[:,j] += (dLdf-1)*X[i]
            else:
                dW[:,j] += dLdf*X[i]
...
def softmax_loss_vectorized(
    ...
    # Loss
    scores = torch.mm(X, W)  # (N,C)
    scores -= torch.max(scores, 1).values[:,None]
    scores_exp = torch.exp(scores)  # (N,C)
    prob_scores = scores_exp / torch.sum(scores_exp, dim=1, keepdim=True)  # (N,C)
    correct_softmax_scores = -torch.log(prob_scores[torch.arange(num_train),y])  # (N,)
    loss += torch.sum(correct_softmax_scores)

    # Gradient
    prob_scores[torch.arange(num_train),y] -= 1
    dW = X.T @ prob_scores
