...
def svm_loss(x, y):
    ...
    # Loss
    correct_class_scores = x[torch.arange(N),y]  # (N,)
    correct_class_scores = correct_class_scores[:,None]  # (N,1)
    margin = x - correct_class_scores.expand_as(x)  # (N,C)
    margin = torch.maximum(torch.zeros_like(margin), margin + torch.ones_like(margin))
    margin[torch.arange(N),y] = 0  # Where j == y[i]
    loss = torch.mean(torch.sum(margin, axis=1))  # sum over classes

    # Gradient
    vals = margin
    vals[vals > 0] = 1  # Where margin > 0
    sums = torch.sum(vals, dim=1)
    dx[margin > 0] = 1
    dx[torch.arange(N),y] -= sums
    dx /= N
    #####################################################
    #                  END OF YOUR CODE                 #
    #####################################################
...
def softmax_loss(x, y):
    ...
    # Loss
    scores_exp = torch.exp(x - torch.max(x, 1).values[:,None])  # (N,C)
    prob_scores = scores_exp / torch.sum(scores_exp, dim=1, keepdim=True)  # (N,C)
    loss = -torch.sum(torch.log(prob_scores[torch.arange(N),y]))
    loss /= N

    # Gradient
    prob_scores[torch.arange(N),y] -= 1
    dx = prob_scores / N
    #####################################################
    #                  END OF YOUR CODE                 #
    #####################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, grads['W2'], grads['b2'] = Linear.backward(d_scores, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params['W'+str(i+1)] = weight_scale * torch.randn(all_dims[i], all_dims[i+1], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if (i == self.num_layers-1):
                scores, cache['c'+str(i+1)] = Linear.forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
            else:
                scores, cache['c'+str(i+1)] = Linear_ReLU.forward(scores, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
                if self.use_dropout:
                    scores, dropout_cache['d'+str(i+1)] = Dropout.forward(scores, self.dropout_param)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if (i == self.num_layers):
                d_scores, grads['W'+str(i)], grads['b'+str(i)] = Linear.backward(d_scores, cache['c'+str(i)])
            else:
                if self.use_dropout:
                    d_scores = Dropout.backward(d_scores, dropout_cache['d'+str(i)])
                d_scores, grads['W'+str(i)], grads['b'+str(i)] = Linear_ReLU.backward(d_scores, cache['c'+str(i)])
...
def create_solver_instance(data_dict, dtype, device, **kwargs):
    ...
    solver = Solver(model, data_dict, **kwargs)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
