...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    ############################################################################
    hidden = X @ W1 + b1
    hidden = torch.clamp(hidden, min=0.0)  # ReLU
    scores = hidden @ W2 + b2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    correct_softmax_scores = -torch.log(prob_scores[torch.arange(N),y])  # (N,)
    ...
    prob_scores /= N  # (N,C)
    # print(b2.shape, h1.shape, scores.shape, W2.shape)
    grads['b2'] = torch.sum(prob_scores, dim=0)  # (C,)
    grads['W2'] = h1.T @ prob_scores + 2*reg*W2  # (H, C)
    dh1 = prob_scores @ W2.T  # (N, H)
    h1[h1 > 0] = 1  # (N, H)
    dh1_relu = h1 * dh1
    grads['b1'] = torch.sum(dh1_relu, dim=0)  # (H, )
    grads['W1'] = X.T @ dh1_relu + 2*reg*W1  # (D, H)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def find_best_net(
    ...
    num_models = len(learning_rates)*len(hidden_sizes)*len(reg_strengths)*len(lr_decays)
