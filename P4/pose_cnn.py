...
class SegmentationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 64):
        ...
        self.conv1 = nn.Conv2d(512, hidden_layer_dim, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(512, hidden_layer_dim, kernel_size=1, stride=1, padding=0)
        self.conv3 = nn.Conv2d(hidden_layer_dim, num_classes+1, kernel_size=1, stride=1, padding=0)
        self.act = nn.ReLU()
    ...
    def forward(self, feature1, feature2):
        ...
        ######################################################################
        conv1 = self.act(self.conv1(feature1))  # (B,hidden_dim,h,w)
        conv2 = F.interpolate(self.act(self.conv2(feature2)), scale_factor=2)  # (B,hidden_dim,h,w)
        probability = F.interpolate(conv1 + conv2, scale_factor=8)  # (B,hidden_dim,H,W)
        probability = F.softmax(self.conv3(probability), dim=1)  # (B,num_classes+1,H,W)
...
class TranslationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 128):
        ...
        self.conv1 = nn.Conv2d(512, hidden_layer_dim, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(512, hidden_layer_dim, kernel_size=1, stride=1, padding=0)
        self.conv3 = nn.Conv2d(hidden_layer_dim, 3*num_classes, kernel_size=1, stride=1, padding=0)
        self.act = nn.ReLU()
    ...
    def forward(self, feature1, feature2):
        ...
        conv1 = self.act(self.conv1(feature1))  # (B,hidden_dim,h,w)
        conv2 = F.interpolate(self.act(self.conv2(feature2)), scale_factor=2)  # (B,hidden_dim,h,w)
        translation = F.interpolate(conv1 + conv2, scale_factor=8)  # (B,hidden_dim,H,W)
        translation = self.conv3(translation)  # (B,3*num_classes,H,W)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
        return translation
...
class RotationBranch(nn.Module):
    ...
    def __init__(self, feature_dim = 512, roi_shape = 7, hidden_dim = 4096, num_classes = 10):
        ...
        self.roipool1 = RoIPool(output_size=roi_shape, spatial_scale=1/8)
        self.roipool2 = RoIPool(output_size=roi_shape, spatial_scale=1/16)
        self.fc1 = nn.Linear(feature_dim*roi_shape**2, hidden_dim)
    ...
    def forward(self, feature1, feature2, bbx):
        ...
        ######################################################################
        pool1 = self.roipool1(feature1, bbx)
        pool2 = self.roipool2(feature2, bbx)
        pools = pool1 + pool2
        quaternion = self.act(self.fc1(pools.flatten(start_dim=1)))
        quaternion = self.act(self.fc2(quaternion))
        quaternion = self.fc3(quaternion).reshape(-1,4*self.num_classes)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class PoseCNN(nn.Module):
    ...
    def __init__(self, pretrained_backbone, models_pcd, cam_intrinsic):
        ...
        self.feature_extractor = FeatureExtraction(pretrained_model=pretrained_backbone).to(device)
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            # predTrans = self.estimateTrans(translation, bbx, segmentation)
            # gtTrans = self.gtTrans(bbx, input_dict)
            # loss_dict['loss_centermap'] = F.l1_loss(predTrans, gtTrans)
            loss_dict['loss_centermap'] = F.l1_loss(translation, input_dict['centermaps'])
            
            selected_bbx = IOUselection(bbx, gt_bbx, self.iou_threshold)
            if len(selected_bbx) != 0:
                quaternion = self.rotation_branch(feature1, feature2, selected_bbx[:,:-1])  # (N_bbx,4)
                pred_Rs, label = self.estimateRotation(quaternion, selected_bbx)
                gt_Rs = self.gtRotation(selected_bbx, input_dict)
            ...
            with torch.no_grad():
                ...
                ######################################################################
                feature1, feature2 = self.feature_extractor(input_dict)
                _, segmentation, bbx = self.segmentation_branch(feature1, feature2)
                bbx = bbx.to(torch.cuda.FloatTensor())
                translation = self.translation_branch(feature1, feature2)
                pred_centers, pred_depths = HoughVoting(segmentation, translation)
                quaternion = self.rotation_branch(feature1, feature2, bbx[:,:-1])
